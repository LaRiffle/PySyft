    ...
    def __call__(self, plan_function):
        ...
        if self.args_shape:
            ...
            try:
                ...
                print('Automatic build using @func2plan failed!\nCheck that:\n'
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.forward is not None:  # if not self.is_built:
            ...
            for i, op in enumerate(self.operations):
                ...
                if return_placeholder is not None:
                    if isinstance(return_placeholder, PlaceHolder):
                        return_placeholder.instantiate(response)
                    elif isinstance(return_placeholder, (list, tuple)):
                        for return_p, rep in zip(return_placeholder, response):
                            if isinstance(return_p, PlaceHolder):
                                return_p.instantiate(rep)
                    else:
                        raise ValueError(f"Response of type {type(response)} is not supported in plan operations")
        ...
        for tag in placeholder.tags:
            if keyword in tag:
                try:
                    ...
                    raise ValueError(f"Tags used in tag_sort should follow the <str>-<int> structure, but found: {tag}")
